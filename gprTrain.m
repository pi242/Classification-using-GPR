function [m_post, cov_post, sd_pointwise] = gprTrain(X, y, x_star, theta)
% dummy = cov_matrix(x_star, X, theta) * inv(cov_matrix(X, X, theta) + (SIGMA_N ^ 2) * eye(size(X, 1
dummy = cov_matrix(x_star, X, theta) * pinv(cov_matrix(X, X, theta));
